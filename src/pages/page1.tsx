...
export default function Page1(props: TProps) {
  return (
    <Layout>
      <h1>Communities</h1>
      <p>In this section of the analysis, the communities are the protagonists. First of all, we will create them and we will visualize them. Then, some statistics are computed in order to understand better their properties. In particular we will study the goodness of the partition, both qualitatively (e.g. inspecting the communities, the number of nodes, the most common tags...) and quantitatively (e.g. degree distribution, correlation matrix, modularity...).</p>
      
      <h2>Creation of the communities</h2>
      <p>Studying the graph, we obtain the result that the score of the modularity is around 0.5, in a range between -1 and +1. This means that there is a high level of modularity, and so the communities are well defined. This is good news as we wanted to find meaningful communities for the further analysis. In Figure (7) it can be seen that there are size communities. In particular there are three communities with quite the same number of nodes (around 700). Then, there are two communities with respectively around 300 nodes more and less. Finally, there is one community with less than 200 nodes.</p>
      <ImageContainer>
      ...
      <p>The figures below shows the communities of the network with their names. It can be seen that each community is identified by a different triplet of tags. This means that each community has, in fact, games with different tags and so the partition is meaningful with respect to the tags.</p>
      <p>The buttons below provides more information on the communities we spotted :</p>
      ...
      <p>From Figure (17) it can be seen the heatmap of the number of links for number of tags. The figure is useful to visualize the effective distribution of the links but it is not so meaningful to understand the actual correlation. In fact, there is a peak of nodes that have around 7 tags and for this reason their absolute amount of links for nodes that have seven tags is bigger than the others. For this reason, a second version of the heatmap is shown in Figure (19). From that figure, the normalized correlation (i.e. the number of links per tag divided by the number of nodes for that tag) is shown. It can be seen that both for very little amount of tags and very big amount of tags, there are many links. In general, however, the normalized number of links for number of tags is more or less constant all over the scale, and this means that there is not a strong correlation. This leads to the fact that the initial choice on how to create a link between edges was a good choice, as it doesn't favor a node by the number of tags.</p>
